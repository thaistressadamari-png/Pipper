...
const AdminPage: React.FC<AdminPageProps> = (props) => {
  ...
  // Controla se é a primeira carga do painel para não notificar pedidos velhos
  ...
  // Função para disparar a notificação via Service Worker (Melhor para iOS)
  const showOrderNotification = async (order: Order) => {
    ...
    const options = {
        ...
        tag: order.id, // ID único evita duplicatas
        vibrate: [200, 100, 200],
        data: { url: '/admin' }
    };
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNewOrders((count, addedOrders) => {
        ...
        // Se for a primeira carga, apenas memorizamos os IDs atuais para não disparar banners
        ...
        // Para cada NOVO pedido detectado após a carga inicial
  ...
  const requestNotificationPermission = async () => {
    if ("Notification" in window) {
      ...
      if (permission === 'granted') {
          // Teste imediato para o usuário ver se funcionou
          const reg = await navigator.serviceWorker.ready;
          reg.showNotification("Alertas Ativados!", {
              body: "Você receberá um banner aqui sempre que chegar um novo pedido.",
              icon: 'https://ugc.production.linktr.ee/fecf1c45-dcf7-4775-8db7-251ba55caa85_Prancheta-1.png?io=true&size=avatar-v3_0'
          });
      }
    }
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white border-r border-gray-200 flex-shrink-0 flex flex-col z-40 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {notificationStatus !== 'granted' && (
          <div className="px-4 mb-2">
            <button onClick={requestNotificationPermission} className="w-full py-2 bg-orange-100 text-orange-700 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-orange-200 hover:bg-orange-200 transition-colors">🔔 Ativar Alertas no iPhone</button>
          </div>
        )}
      ...
      <div className="flex-grow flex flex-col min-w-0">
        <header className="bg-white shadow-sm h-16 flex-shrink-0">
            <div className="container mx-auto px-4 h-full flex justify-between items-center gap-4">
                ...
                <button onClick={props.onNavigateBack} className="text-sm font-medium text-brand-primary hover:underline whitespace-nowrap">&larr; Voltar ao cardápio</button>
            </div>
        </header>
